...
CREATE TYPE outcome_type AS ENUM ('TOO_COMPLEX', 'DECONTROL', 'CONTROL_ENTRY_FOUND');
...
CREATE TABLE stage_answer (
  ...
  go_to_outcome_type  outcome_type,
  control_entry_id    BIGINT,
  answer_text         TEXT,
...
ALTER TABLE stage
  ...
  (question_type = 'DECONTROL' AND next_stage_id IS NOT NULL) OR question_type != 'DECONTROL'
);
...
ALTER TABLE stage_answer
  ...
  ADD CONSTRAINT stage_answer_ck_1 CHECK (
    (go_to_stage_id IS NULL AND go_to_outcome_type IS NOT NULL)
    OR

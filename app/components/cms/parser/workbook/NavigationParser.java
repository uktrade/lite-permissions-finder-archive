...
import static utils.StringUtils.pascalCaseText;
...
public class NavigationParser {
  ...
  public static List<NavigationLevel> parse(Workbook workbook) {
    ...
    for (int i = 2; i < workbook.getNumberOfSheets(); i++) {
      ...
      // Pull sheet name from spreadsheet
      String sheetName = pascalCaseText(sheet.getSheetName());
      String friendlySheetName = sheet.getSheetName();

      // Create a base NavigationLevel that can hold all parsed in
      NavigationLevel rootNavigationLevel = new NavigationLevel("ROOT", "ROOT", -1, sheetName, friendlySheetName);
      ...
      for (int rowIndex = RowIndices.NAVIGATION_START; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
        ...
        for (int navColIndex = ColumnIndices.Navigation.START; navColIndex <= ColumnIndices.Navigation.END; navColIndex++) {
          ...
          NavigationLevel navigationLevel = new NavigationLevel(navCellAddress, navCellValue, currIndentLevel, sheetName, friendlySheetName);

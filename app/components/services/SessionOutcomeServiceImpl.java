...
public class SessionOutcomeServiceImpl implements SessionOutcomeService {
  ...
  public void generateItemListedOutcome(String userId, String sessionId,
                                        String controlEntryId) throws TooManyCustomersOrSitesException {
  ...
  private String generateLetter(String userId, String sessionId, String resumeCode, OutcomeType outcomeType,
                                Html nlrBreadcrumb, String description) throws TooManyCustomersOrSitesException {
    CustomerView customerView = getCustomerId(userId);
    String customerId = customerView.getCustomerId();
    SiteView siteView = getSite(customerId, userId);
  ...
  private SiteView getSite(String customerId, String userId) throws TooManyCustomersOrSitesException {
    ...
    if (optSites.isPresent()) {
      ...
      if (sites.size() == 1) {
        ...
        throw new TooManyCustomersOrSitesException("Expected user [" + userId + "] to only have 1 associated Site but found: " + sites.size());
      }
    }
    throw new TooManyCustomersOrSitesException("Not a single Site associated with user/customer: " + userId + "/" + customerId);
  }

  private CustomerView getCustomerId(String userId) throws TooManyCustomersOrSitesException {
    ...
    if (optCustomers.isPresent()) {
      ...
      if (customers.size() == 1) {
        ...
        throw new TooManyCustomersOrSitesException("Expected user [" + userId + "] to only have 1 associated Customer but found: " + customers.size());
      }
    }
    throw new TooManyCustomersOrSitesException("Not a single Customer associated with user: " + userId);
  }
